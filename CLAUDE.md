...
GitHub Actions runs lint, typecheck, and test in parallel on every push. Release workflow triggers on semver tag push (pattern `[0-9]*.[0-9]*.[0-9]*`), verifies the tag is on master, builds, extracts the matching section from `CHANGELOG.md` via `scripts/extract-release-notes.awk`, and publishes a GitHub release with that section as the body. `vite.config.js` derives minification and sourcemaps from Vite's `mode` (and pins `NODE_ENV=production` for it), so `npm run build` from a clean checkout reproduces the released `main.js` byte for byte no matter what the build environment presets — Obsidian's reviewer diffs its own build against the release asset. `npm run dev` opts into the dev build via `--mode development`. Each release attaches only `main.js` and `manifest.json` (Obsidian's reviewer flags any other asset as unsupported) and records a build-provenance attestation for `main.js` via `actions/attest-build-provenance`.
